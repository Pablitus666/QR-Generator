...
def _require(tkroot):
    ...
    try:
        ...
        # 1. Intentamos usar la variable de entorno que solemos configurar
        tkdnd_lib_path = os.environ.get('TKDND_LIBRARY')
        
        # 2. Si no hay variable, intentamos localizarla en el bundle
        if not tkdnd_lib_path and hasattr(sys, '_MEIPASS'):
            # Intentamos las dos ubicaciones comunes
            possible_paths = [
                os.path.join(sys._MEIPASS, 'tkdnd', tkdnd_platform_rep),
                os.path.join(sys._MEIPASS, 'tkinterdnd2', 'tkdnd', tkdnd_platform_rep)
            ]
        ...
        # 3. Fallback a la ubicación estándar
        if not tkdnd_lib_path:
            tkdnd_lib_path = os.path.join(os.path.dirname(__file__), 'tkdnd', tkdnd_platform_rep)
        
        # Limpiamos la ruta para Tcl (crucial en Windows)
        tkdnd_lib_path = tkdnd_lib_path.replace('\\', '/')
        
        # Inyectamos en el auto_path de Tcl
